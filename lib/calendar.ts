...
//
// The grid math, month paging, and weekday layout now live in the calendar
// library (see components/ui/calendar.tsx) — there is no reason to hand-roll
// that. What stays here is the boundary logic the library does *not* cover: the
// strict `YYYY-MM-DD` ↔ `Date` round-trip the form contract depends on. All
// math uses the local-time `Date(year, monthIndex, day)` constructor and the
// local getters — never `Date.toISOString()`, which would shift the day across
// time zones. Keeping it here (not in the component) makes it unit testable
// without a DOM, per docs/ARCHITECTURE.md.
