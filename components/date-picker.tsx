...
/**
 * A calendar date picker that posts its value through a hidden input, so it is a
 * drop-in replacement for `<input type="date" name=... />` inside a form. The
    ...
    <PopoverPrimitive.Root open={open} onOpenChange={setOpen}>
      ...
      <PopoverPrimitive.Portal>
        <PopoverPrimitive.Positioner side="bottom" align="start" sideOffset={4}>
          <PopoverPrimitive.Popup className="z-50 origin-(--transform-origin) rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 outline-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95">
            <Calendar
              mode="single"
              selected={date}
              defaultMonth={date}
